...
box = "(box :class 'workspaces' :orientation 'horizontal' :halign 'start' :spacing 4 "
...
for i = 1, #existing_workspaces do
	...
	if tonumber(c) == tonumber(active_workspace) then
		...
		local btn = '(button :class "active" :onclick "hyprctl dispatch workspace ' .. c .. ' " "' .. c .. '")'
		box = box .. btn
	elseif c ~= "\n" then
		-- local btn = '(button :class "inactive" :onclick "hyprctl dispatch workspace ' .. c .. '" "")'
		local btn = '(button :class "inactive" :onclick "hyprctl dispatch workspace ' .. c .. '" "' .. c .. '")'
		box = box .. btn
	end
end
